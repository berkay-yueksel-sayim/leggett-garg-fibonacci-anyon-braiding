...
random word tested---a sharp split that mirrors the Clarke--Sau--Das-Sarma
...
Third, the Fourier spectrum of the envelope
$K_{3,\max}(\delta) = \max_{|w| \leq L} K_{3}(\delta;\,w)$ is
dominated by the $k=3$ harmonic (period $2\pi/3$);
each individual fixed-word $K_{3}(\delta;\,w)$ has the
algebraically expected $2\pi/6$ period from the six $R$-matrix
insertions in $(\sigma_{1}\sigma_{2})^{3}$, but the envelope shows
optimal-word reshuffling on a finer scale
(\emph{correcting the $k=6$ envelope value reported in v1.0,
which does not reproduce under larger search-space sanity checks
at $L_{\max} \in \{7, 8\}$}). We also find that $K_{3}$
...
\begin{quote}\small                                                                %% v1.1
\textbf{Note (v1.1, 2026-05-28):} This is a corrected version of the              %% v1.1
letter previously released as v1.0 on 25.5.2026 (Zenodo Concept-DOI               %% v1.1
\href{https://doi.org/10.5281/zenodo.20372744}{10.5281/zenodo.20372744}).         %% v1.1
The corrections are: (i)~the dominant Fourier harmonic of                          %% v1.1
$K_{3,\max}(\delta)$ is corrected from $k=6$ to $k=3$ in the abstract,             %% v1.1
overview, figure caption, and §III.D body (the fixed-word $2\pi/6$                 %% v1.1
algebraic identity is preserved as a separate statement; see §III.D for            %% v1.1
the full reformulation); (ii)~the bibliography entry previously labeled            %% v1.1
``Hou \emph{et al.}'' (Phys.\ Rev.\ X \textbf{6}, 021005) is corrected             %% v1.1
to its true attribution Clarke, Sau, and Das~Sarma, with the body text             %% v1.1
``Hou--Shtengel split'' updated to ``Clarke--Sau--Das-Sarma split''                %% v1.1
throughout; (iii)~the Sorella~2023 entry is updated with the verified              %% v1.1
title and Foundations of Physics vol/article citation; (iv)~the                    %% v1.1
Minev~2025 entry is updated with the full author list, correct article             %% v1.1
number 6225, and arXiv link. No numerical results, no figures, and no              %% v1.1
other body claims are affected; the corrections are bibliographic and              %% v1.1
the Fourier-period reformulation is a sharpening (envelope vs                      %% v1.1
fixed-word distinction), not a result change.                                      %% v1.1
\end{quote}                                                                        %% v1.1
...
additional non-Clifford gate~\cite{ClarkeSauDasSarma2016}. The two models
...
braiding         & Clifford only      & \textbf{universal} \\
...
(iv)~a $2\pi/3$ dominant Fourier period of the envelope
$K_{3,\max}(\delta)$ over all braid words at fixed length; the
algebraically expected $2\pi/6$ period from six $R$-matrix
insertions in $(\sigma_{1}\sigma_{2})^{3}$ applies to each
individual fixed word but is not the dominant mode of the envelope
(correcting the v1.0 statement; see Sec.~\ref{sec:results}, §III.D);
...
$L \leq 9$, fine sampling. The dominant Fourier harmonic of the
envelope is $k=3$, period $2\pi/3$, reflecting optimal-word
reshuffling on a finer scale than the $2\pi/6$ period algebraically
expected for each fixed word from the six $R$-matrix insertions in
$(\sigma_{1}\sigma_{2})^{3}$ (correcting the $k=6$ envelope value
reported in v1.0; the $k=3 / k=6$ amplitude ratio is stable at
$1.061 \to 1.063$ between $L_{\max} = 7$ and $L_{\max} = 8$
sanity re-runs, so the dominance is robust against
$L_{\max}$, not a search-budget artefact). The $\mathrm{Z}_{5}$
period $2\pi/5$ of the central element is not the dominant mode. The
...
This is the temporal counterpart of the Clarke--Sau--Das-Sarma
result~\cite{ClarkeSauDasSarma2016} for the spatial CHSH inequality: in both
...
$K_{3,\max}(\delta) = \max_{|w| \leq L} K_{3}(\delta;\,w)$ is
dominated by the harmonic $k=3$, i.e.\ period $2\pi/3$, with all top
harmonics being multiples of three (top five: $k = 3, 6, 9, 12, 15$).
The algebraically expected $2\pi/6$ period applies to each
\emph{fixed} braid word $w$: the central word
$(\sigma_{1}\sigma_{2})^{3}$ consists of six generator insertions,
each of which carries the sector phase $\delta$ once through
$R_{\tau} \mapsto R_{\tau}\,e^{i\delta}$, so any \emph{fixed-word}
$K_{3}(\delta;\,w)$ is invariant under
$\delta \mapsto \delta + 2\pi/6$. The envelope $K_{3,\max}(\delta)$,
however, shows the optimal winner-word $w^{\star}(\delta)$
reshuffling on a finer scale --- the dominant mode is $k=3$, with
$k=6$ as the second-strongest. The structurally expected
$\mathrm{Z}_{5}$ harmonic (period $2\pi/5$, corresponding to the
central element $(\sigma_{1}\sigma_{2})^{3}$ as a scalar of order
$5$) is \emph{not} the dominant mode of $K_{3,\max}(\delta)$.

\emph{Correction to v1.0:} The first release of this letter reported
$k=6$ as the dominant envelope harmonic; that statement does not
reproduce under larger-search-space sanity checks. Independent
reproducer runs at $L_{\max} = 7$ ($n_{\delta} = 120$, walltime
$\approx 2.4$ min) and a pre-v1.1 sanity re-run at $L_{\max} = 8$
($4^{8} = 65{,}536$ words, walltime $\approx 10.4$ min) both find
$k=3$ as the dominant envelope harmonic; the $k=3 / k=6$ amplitude
ratio is stable at $1.061 \to 1.063$, indicating the dominance is
robust against $L_{\max}$ and not a search-budget artefact. The
fixed-word $2\pi/6$ algebraic identity and the envelope $2\pi/3$
harmonic are not in conflict: they refer to quantitatively
different objects (fixed-word $K_{3}(\delta;\,w)$ vs envelope
$K_{3,\max}(\delta)$), and the present letter is concerned with
the latter.
...
Clarke, Sau, and Das~Sarma~\cite{ClarkeSauDasSarma2016} established that
...
braid word between measurements. The Clarke--Sau--Das-Sarma split predicts
